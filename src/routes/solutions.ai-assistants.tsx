...
function MissedCallIllustration() {
  // iOS-style missed call notification card
  return (
    <IllustrationFrame>
      <div
...
function SlowReplyIllustration() {
  // Inbox UI with stale unread rows
  const rows = [
...
function AfterHoursIllustration() {
  // 24-hour ring split between business hours (light) and after-hours (dark with accent)
  const R = 46;
  const C = 2 * Math.PI * R;
  // business hours 9am–5pm = 8h of 24h => 1/3 of circle
    ...
    <IllustrationFrame>
      <div className="flex items-center gap-4">
        <svg viewBox="-60 -60 120 120" className="h-28 w-28">
          ...
          {/* business hours arc (light blue) — starts at top, runs 1/3 */}
          <circle
            r={R}
            fill="none"
            stroke="#CBD5E1"
            ...
            transform="rotate(-90)"
          />
          {/* after-hours arc (accent) — remaining 2/3 */}
          <circle
            r={R}
            fill="none"
            stroke="#2563EB"
